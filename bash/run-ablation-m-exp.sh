...
mlist="1000 2000 3000 4000 5000 6000 7000 8000 9000 10000"
...
ILTS+iter+ch+avg+rec(ns),A_METADATAS(ns),B_CHUNK(ns),readChunkNum,traversedPointNum"\
 > $HOME_PATH/res-ablation-m.csv
sed '1d' tmp4.csv >> $HOME_PATH/res-ablation-m.csv
rm tmp*.csv
...
sed -i -e 1's/^/m,estimated chunks per interval,/' $HOME_PATH/res-ablation-m.csv
line=2

for m in $mlist
do
  c=$((echo scale=3 ; echo ${TOTAL_POINT_NUMBER}/${IOTDB_CHUNK_POINT_SIZE}/$m) | bc )
  sed -i -e ${line}"s/^/${m},${c},/" $HOME_PATH/res-ablation-m.csv
  let line+=1
done
