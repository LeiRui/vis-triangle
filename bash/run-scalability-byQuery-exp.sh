...
$HOME_PATH/tool.sh system_dir $HOME_PATH/dataSpace_100/${DATASET}_O_10_D_0_0/system ../../iotdb-engine-example.properties
$HOME_PATH/tool.sh data_dirs $HOME_PATH/dataSpace_100/${DATASET}_O_10_D_0_0/data ../../iotdb-engine-example.properties
$HOME_PATH/tool.sh wal_dir $HOME_PATH/dataSpace_100/${DATASET}_O_10_D_0_0/wal ../../iotdb-engine-example.properties
...
cd $HOME_PATH/${DATASET}_testspace/O_10_D_0_0/vary_queryRange
mkdir $approach
cd $approach
...
do
  ...
  then
    ...
    $HOME_PATH/query_experiment.sh root "${OM3_MIN_DEVICE}.${MEASUREMENT},${OM3_MAX_DEVICE}.${MEASUREMENT}" \
            ...
            ${DATA_MAX_TIME} ${range} ${FIX_M} $approach \
            "${TRI_VISUALIZATION_EXP}/tools/varyN-byQuery/ids-${DATASET,,}-${per}.csv" \
            >> result_${i}.txt
  else # default rep
    # Note the following command print info is appended into result_${i}.txt for query latency exp
    ...
    $HOME_PATH/query_experiment.sh ${DEVICE} ${MEASUREMENT} ${TIMESTAMP_PRECISION} ${DATA_MIN_TIME} \
      ${DATA_MAX_TIME} ${range} ${FIX_M} $approach >> result_${i}.txt
  fi
...
ILTS(ns),A_METADATAS(ns),B_CHUNK(ns),readChunkNum,traversedPointNum"\
 > $HOME_PATH/res-${DATASET}-scalability-byQuery.csv

sed '1d' tmp5.csv >> $HOME_PATH/res-${DATASET}-scalability-byQuery.csv
rm tmp*.csv
...
sed -i -e 1's/^/range,estimated chunks per interval,/' $HOME_PATH/res-${DATASET}-scalability-byQuery.csv
line=2
for per in $perlist
do
  range=$((echo scale=0 ; echo ${per}*${TOTAL_TIME_RANGE}/100) | bc )
  c=$((echo scale=0 ; echo ${TOTAL_POINT_NUMBER}/${IOTDB_CHUNK_POINT_SIZE}/${FIX_M}*${per}/100) | bc )
  sed -i -e ${line}"s/^/${range},${c},/" $HOME_PATH/res-${DATASET}-scalability-byQuery.csv
  let line+=1
done
