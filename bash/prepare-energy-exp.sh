...
do
  ...
  $HOME_PATH/tool.sh DATASET Pyra1 run-${value}-exp.sh
  $HOME_PATH/tool.sh DEVICE "root.Pyra1.targetDevice" run-${value}-exp.sh
  ...
  $HOME_PATH/tool.sh INPUT_DATA_PATH ${TRI_VISUALIZATION_EXP}/datasets/Pyra1.csv run-${value}-exp.sh
done;

echo "prepare testspace directory";
cd $HOME_PATH
mkdir Pyra1_testspace
