...
class SessionDataSet(object):
    ...
    def todf_noFetch(self):
        return resultset_to_pandas_noFetch(self)

    def has_next_noFetch(self):
        return self.iotdb_rpc_data_set.next_noFetch()
...
def resultset_to_pandas_noFetch(result_set: SessionDataSet) -> pd.DataFrame:
    """
    Transforms a SessionDataSet from IoTDB to a Pandas Data Frame
    Each Field from IoTDB is a column in Pandas
    :param result_set:
    ...
    while result_set.has_next_noFetch():
        record = result_set.next()
