...
class TSExecuteFinishResp(object):
    """
    Attributes:
     - status
     - queryId
    ...
    def __init__(self, status=None, queryId=None, executionInfo=None,):
        self.status = status
        self.queryId = queryId
    ...
    def read(self, iprot):
        ...
        while True:
            ...
            if fid == 1:
                ...
                if ftype == TType.STRING:
                    self.executionInfo = iprot.readString().decode('utf-8', errors='replace') if sys.version_info[0] == 2 else iprot.readString()
                else:
                    iprot.skip(ftype)
            else:
                iprot.skip(ftype)
    ...
    def write(self, oprot):
        if oprot._fast_encode is not None and self.thrift_spec is not None:
            oprot.trans.write(oprot._fast_encode(self, [self.__class__, self.thrift_spec]))
            return
        oprot.writeStructBegin('TSExecuteFinishResp')
        if self.status is not None:
            oprot.writeFieldBegin('status', TType.STRUCT, 1)
            self.status.write(oprot)
            oprot.writeFieldEnd()
        ...
        if self.executionInfo is not None:
            oprot.writeFieldBegin('executionInfo', TType.STRING, 3)
            oprot.writeString(self.executionInfo.encode('utf-8') if sys.version_info[0] == 2 else self.executionInfo)
            oprot.writeFieldEnd()
        oprot.writeFieldStop()
        oprot.writeStructEnd()
    ...
    def validate(self):
        if self.status is None:
            raise TProtocolException(message='Required field status is unset!')
        if self.executionInfo is None:
            raise TProtocolException(message='Required field executionInfo is unset!')
        return
...
all_structs.append(TSExecuteFinishResp)
TSExecuteFinishResp.thrift_spec = (
    None,  # 0
    (1, TType.STRUCT, 'status', [TSStatus, None], None, ),  # 1
    (2, TType.I64, 'queryId', None, None, ),  # 2
    (3, TType.STRING, 'executionInfo', 'UTF8', None, ),  # 3
)
