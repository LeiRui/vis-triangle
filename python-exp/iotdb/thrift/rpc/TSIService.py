...
class Client(Iface):
    ...
    def executeFinish(self):
        self.send_executeFinish()
        return self.recv_executeFinish()

    def send_executeFinish(self):
        self._oprot.writeMessageBegin('executeFinish', TMessageType.CALL, self._seqid)
        args = executeFinish_args()
        args.write(self._oprot)
        self._oprot.writeMessageEnd()
        self._oprot.trans.flush()

    def recv_executeFinish(self):
        iprot = self._iprot
        (fname, mtype, rseqid) = iprot.readMessageBegin()
        if mtype == TMessageType.EXCEPTION:
            x = TApplicationException()
            ...
            iprot.readMessageEnd()
            raise x
        result = executeFinish_result()
        result.read(iprot)
        iprot.readMessageEnd()
        if result.success is not None:
            return result.success
        raise TApplicationException(TApplicationException.MISSING_RESULT, "executeFinish failed: unknown result")
...
class Processor(Iface, TProcessor):
    def __init__(self, handler):
        ...
        self._processMap["executeFinish"] = Processor.process_executeFinish
    ...
    def process_executeFinish(self, seqid, iprot, oprot):
        args = executeFinish_args()
        args.read(iprot)
        iprot.readMessageEnd()
        result = executeFinish_result()
        try:
            result.success = self._handler.executeFinish()
            msg_type = TMessageType.REPLY
        except TTransport.TTransportException:
            raise
        except TApplicationException as ex:
            ...
            msg_type = TMessageType.EXCEPTION
            result = TApplicationException(TApplicationException.INTERNAL_ERROR, 'Internal error')
        oprot.writeMessageBegin("executeFinish", msg_type, seqid)
        result.write(oprot)
        oprot.writeMessageEnd()
        oprot.trans.flush()
...
class executeFinish_args(object):
    ...
    def write(self, oprot):
        if oprot._fast_encode is not None and self.thrift_spec is not None:
            oprot.trans.write(oprot._fast_encode(self, [self.__class__, self.thrift_spec]))
            return
        oprot.writeStructBegin('executeFinish_args')
        oprot.writeFieldStop()
        oprot.writeStructEnd()
    ...
    def __ne__(self, other):
        return not (self == other)
all_structs.append(executeFinish_args)
executeFinish_args.thrift_spec = (
)


class executeFinish_result(object):
    """
    Attributes:
     - success
    ...
    def read(self, iprot):
        ...
        while True:
            ...
            if fid == 0:
                if ftype == TType.STRUCT:
                    self.success = TSExecuteFinishResp()
                    self.success.read(iprot)
                else:
                    iprot.skip(ftype)
            else:
    ...
    def write(self, oprot):
        if oprot._fast_encode is not None and self.thrift_spec is not None:
            oprot.trans.write(oprot._fast_encode(self, [self.__class__, self.thrift_spec]))
            return
        oprot.writeStructBegin('executeFinish_result')
        if self.success is not None:
            oprot.writeFieldBegin('success', TType.STRUCT, 0)
            self.success.write(oprot)
            oprot.writeFieldEnd()
    ...
    def __ne__(self, other):
        return not (self == other)
all_structs.append(executeFinish_result)
executeFinish_result.thrift_spec = (
    (0, TType.STRUCT, 'success', [TSExecuteFinishResp, None], None, ),  # 0
)
